...
public strictfp class RobotPlayer {
	...
	/*
	 * Stores what needs to be built They should be added in as influence*10 + type,
	 * where slanderer = 1, politician = 2, muckraker =3 -Winston
	 */
	...
	/*
	 * Runs Stage One of the Strategy: 3 Slanderer - 41 Influence (Total 123
	 * Influence, 2 initially and 3rd is staggered) 1 Politician - 25 Influence 3
	 * Politicians - 11 Influence Bidding 1 for vote - Winston
	 */
	...
	/*
	 * Sets all the previous variables to current round settings. To be used right
	...
	/**
	 * To be run as an enlightenment center Also updates slanderers to politicians
	 * in the local list Returns integer array of first the cnt, and then the
	 * direction of the first slanderer detected
	 * 
	 * @param type The type of robot to be counted -Ben
	 */
				...
				if (rc.canGetFlag(id))
					if (rc.canSenseRobot(id) && rc.senseRobot(id).getType().equals(RobotType.SLANDERER)) {
						...
						if (rc.getFlag(id) >= 11 && rc.getFlag(id) <= 14) {
							if (rc.getFlag(rc.getID()) % 10 == 0) {// We find direction of slanderer if we haven't set
																	// one yet
	 ...
	 * @param m   muckracker cnt 2^24=1 67 77 21 6 p s m dir
	 * @param dir 1 means top left, 2 top right, 3 bottom right, 4 bottom left -Ben
	 */
	...
	/**
	 * Code for non-enlightenment center units Finds the ID of the enlightenment
	 * center that created it and sets that to a proper number -Ben
	 */
